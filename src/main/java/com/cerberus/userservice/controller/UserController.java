...
public class UserController {
    ...
    @PatchMapping("/{id}")
    public HttpStatus update(@PathVariable("id") Long id, @RequestBody UserDto userDto, BindingResult bindingResult){
        if(bindingResult.hasFieldErrors()){
            String errors = bindingResult.getFieldErrors().stream().map(fieldError ->
                    fieldError.getField() + " " + fieldError.getDefaultMessage()).toList().toString();
